...
app = Flask(__name__)
# CORS explícito para tu dominio de Vercel
...
# Depuración: si faltan credenciales, usa valores seguros
...
def get_equipos_todos():
    """Devuelve SOLO los equipos activos (estado_eq = 'A')"""
    if not supabase:
        return jsonify({"error": "Backend no configurado"}), 500
    try:
    ...
    for qr in codigos_qr:
        try:
            ...
            if not equipo_resp.data or len(equipo_resp.data) == 0:
                print(f"⚠️ Equipo no encontrado: {qr}")
                continue
            ...
            # Marcar movimientos anteriores como históricos ('B')
            supabase.table('movimientos').update({'estado_mv': 'B'}).eq('equipo_id_mv', equipo_id).execute()

            # Insertar nuevo movimiento activo ('A')
            nuevo_mov = {
            ...
            print(f"✅ Equipo {qr} movido a {destino}")
